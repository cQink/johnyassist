...
@pytest.mark.parametrize("время,пора", [
    ...
    ("11:00", True),      # догоняющий запуск: утренние cron потерялись
    ("15:00", True),      # уроки и меню на сегодня нужны до конца учебного дня
    ("15:31", False),     # учебный день кончился, сводка про него не нужна
    ("06:30", False),     # ещё рано
])
def test_окно_запуска(время, пора):
    ...
    assert digest.should_run(now, "07:30", digest._WINDOW_HOURS["morning"]) is пора


@pytest.mark.parametrize("время,опоздали", [
    ("07:30", False),     # вовремя
    ("08:15", False),     # штатная задержка cron — это не опоздание
    ("09:30", True),      # два часа: «Доброе утро» уже звучит поломкой
    ("14:00", True),      # догоняющий запуск в конце окна
])
def test_опоздавшую_утреннюю_не_называем_добрым_утром(время, опоздали):
    """Окно тянется до 15:30 — значит приветствие обязано зависеть от часа.

    Без этого широкое окно чинило доставку ценой доверия: «Доброе утро» в два
    часа дня читается как сломанный бот, и остальному тексту тоже не верят.
    """
    час, минута = (int(x) for x in время.split(":"))
    now = dt.datetime(2026, 8, 24, час, минута, tzinfo=СТОКГОЛЬМ)
    assert digest.is_late(now, "07:30") is опоздали

    текст = digest.build("morning", [], "+12…+21", ДЕНЬ, notable=True, late=опоздали)
    начало = текст.splitlines()[0]
    assert начало.startswith("Доброе утро") is not опоздали
    # Дата остаётся на месте в обоих случаях: без неё непонятно, за какой день
    # сводка, а это как раз то, что нужно знать у опоздавшей.
    assert "24 августа" in начало


def test_вечернюю_опоздавшей_не_считаем():
    """У неё нет приветствия по времени суток — «Завтра, …» верно и в 23:59."""
    поздно = dt.datetime(2026, 8, 24, 23, 30, tzinfo=СТОКГОЛЬМ)
    текст = digest.build("evening", [], "+12…+21", ДЕНЬ, notable=True, late=True)
    assert текст.splitlines()[0].startswith("Завтра,")
    assert digest.is_late(поздно, "21:00") is True


def test_вечернее_окно_не_переходит_за_полночь():
    """Иначе «уже отправляли сегодня» сравнивалось бы с другой датой.

    Вечерняя сводка про завтра, отправленная в 00:30, пометила бы уже
    наступивший день — и настоящая вечерняя того дня не пришла бы.
    """
    поздно = dt.datetime(2026, 8, 25, 0, 30, tzinfo=СТОКГОЛЬМ)
    assert digest.should_run(поздно, "21:00", digest._WINDOW_HOURS["evening"]) is False


def test_зимой_и_летом_окно_ловит_нужный_час():
    """Переход на летнее время: cron в UTC не знает про него, digest знает.

    Утренние cron стоят на 05:30…10:30 UTC. Летом Стокгольм это UTC+2, зимой
    UTC+1 — значит один и тот же запуск в 05:30 UTC попадает в окно летом
    (07:30 местного) и НЕ попадает зимой (06:30, ещё рано). Если этот тест
    покраснеет, значит сводка приходит на час не вовремя.
    """
    опции = {"morning": "07:30", "evening": "21:00"}
    летом = dt.datetime(2026, 7, 15, 5, 30, tzinfo=dt.timezone.utc).astimezone(СТОКГОЛЬМ)
    зимой = dt.datetime(2026, 1, 15, 5, 30, tzinfo=dt.timezone.utc).astimezone(СТОКГОЛЬМ)
    assert digest.choose_kind(летом, опции) == "morning"
    assert digest.choose_kind(зимой, опции) is None
    # Зимой сработает следующий по счёту cron — в 06:30 UTC это ровно 07:30.
    зимой_позже = dt.datetime(2026, 1, 15, 6, 30, tzinfo=dt.timezone.utc).astimezone(СТОКГОЛЬМ)
    assert digest.choose_kind(зимой_позже, опции) == "morning"


# -- защита от повторной отправки: cron теперь стоит каждый час --

def test_вторая_сводка_за_день_не_отправляется():
    """Cron стоит раз в час, значит без отметки человек получил бы пять копий.

    Именно отметка, а не узкое окно, защищает от повторов — окно широкое
    намеренно, чтобы пережить потерянный запуск.
    """
    опции = {"morning": "07:30", "evening": "21:00"}
    now = dt.datetime(2026, 8, 24, 9, 30, tzinfo=СТОКГОЛЬМ)
    assert digest.choose_kind(now, опции, {}) == "morning"
    assert digest.choose_kind(now, опции, {"morning": "2026-08-24"}) is None


def test_вчерашняя_отметка_сегодня_не_мешает():
    опции = {"morning": "07:30", "evening": "21:00"}
    now = dt.datetime(2026, 8, 24, 7, 35, tzinfo=СТОКГОЛЬМ)
    assert digest.choose_kind(now, опции, {"morning": "2026-08-23"}) == "morning"


def test_утренняя_отметка_не_мешает_вечерней():
    опции = {"morning": "07:30", "evening": "21:00"}
    now = dt.datetime(2026, 8, 24, 21, 10, tzinfo=СТОКГОЛЬМ)
    assert digest.choose_kind(now, опции, {"morning": "2026-08-24"}) == "evening"


def test_потерянный_утренний_запуск_догоняется():
    """Ровно тот случай, ради которого всё переделано (27.08.2026).

    GitHub не создал ни одного запуска в 05:30 и 06:30 UTC, и сводка не пришла
    вовсе. Теперь запуск в 10:30 утра увидит, что за сегодня не отправляли, и
    пришлёт — с опозданием, но пришлёт.
    """
    опции = {"morning": "07:30", "evening": "21:00"}
    догоняющий = dt.datetime(2026, 8, 24, 10, 30, tzinfo=СТОКГОЛЬМ)
    assert digest.choose_kind(догоняющий, опции, {"morning": "2026-08-23"}) == "morning"


def test_отметка_переживает_запись_и_чтение(tmp_path):
    путь = tmp_path / "last-sent.json"
    digest.save_state(путь, {"morning": "2026-08-24"})
    assert digest.load_state(путь) == {"morning": "2026-08-24"}


def test_нет_отметки_значит_не_отправляли(tmp_path):
    assert digest.load_state(tmp_path / "нет.json") == {}


def test_битая_отметка_не_отменяет_сводку(tmp_path):
    # Лишняя сводка несравнимо лучше, чем не пришедшая из-за битого файла.
    путь = tmp_path / "last-sent.json"
    путь.write_text("{это не json", encoding="utf-8")
    assert digest.load_state(путь) == {}
...
def test_вечером_до_срока_не_запускаемся():
    """Промежуток между окнами: утреннее уже закрылось, вечернее ещё не открылось.

    Час выбран с запасом: утреннее окно тянется до 15:30, и запуски по cron
    идут до 15:37 — поэтому «ни то ни сё» начинается только после этого.
    """
    now = dt.datetime(2026, 8, 24, 17, 0, tzinfo=СТОКГОЛЬМ)
