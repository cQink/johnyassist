...
# не тот факт.
#
# calendar — то же самое, что list_memory: на «что у меня завтра» ответом
# является сам список, и «дзынь» вместо него не значит ничего. Живой промах
# 2026-08-23: команда отработала правильно, а человек услышал «Так точно».
#
# remind — по причине forget/update_fact, а не list_memory. Его сообщение
# несёт РАСПОЗНАННУЮ ДАТУ («Записал на четверг, 27 августа»), и это
# единственная возможность заметить, что фразу разобрали не так. Под звуком
# страховка не работает, а напоминание молча уезжает не на тот день — что
# обнаруживается тем, что человек не пришёл к врачу.
_SPOKEN_ACTIONS = frozenset({"list_memory", "forget", "update_fact", "calendar", "remind"})
