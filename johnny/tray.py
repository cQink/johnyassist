...
def main() -> None:
    ...
    def restart_app(_icon, _item) -> None:
        ...
        guard.stop()  # join внутри stop() дожидается конца текущей подмены —
        # иначе старый сторож успел бы доиграть use_model уже после того, как
        # новый Джони начал грузить модели на ту же карту
        controller.stop()
        stop_event.set()
        icon.stop()
