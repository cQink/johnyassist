...
# Промпт просит ОБЪЯСНИТЬ блюдо, а не перевести название, и это разные вещи.
# Первая версия велела «сохранять узнаваемые названия» — и выдавала «чили кон
# карне с рисом». Владелец справедливо заметил (26.08.2026), что это не перевод:
# из такой строки не понять даже, мясное блюдо или нет. Теперь модель обязана
# назвать продукты: «острое рагу из говяжьего фарша с фасолью и рисом».
_PROMPT = (
    "Ты объясняешь по-русски, что дают на обед в шведской школе.\n"
    "Скажи ПРОСТЫМИ СЛОВАМИ, из чего блюдо: какое мясо или рыба, какой гарнир.\n"
    "НЕ транслитерируй иностранные названия — «чили кон карне» ничего не "
    "объясняет, напиши «острое рагу из говяжьего фарша с фасолью».\n"
    "Одна короткая строка, без кавычек, без пояснений в скобках, без вводных "
    "слов.\n\nБлюдо: {dish}"
)
...
def _clean(ответ: str) -> str:
    ...
    Точка в конце срезается: остальные строки сводки без неё, и одна фраза с
    точкой посреди списка выглядит опечаткой. Многоточие не трогаем — оно
    что-то значит, а одиночная точка нет.
    """
    for строка in (ответ or "").splitlines():
        строка = строка.strip().strip('"«»').strip()
        if строка.endswith(".") and not строка.endswith(".."):
            строка = строка[:-1].rstrip()
        if строка:
            return строка
    return ""
...
def _sane(перевод: str, оригинал: str) -> bool:
    ...
    Порог щедрый вдвойне: кириллица длиннее латиницы, а объяснение длиннее
    названия — «Kebabgryta med ris» это восемнадцать знаков, а «тушёное мясо
    кебаб с рисом» уже двадцать девять. Но вчетверо длиннее оригинала не бывает
    и объяснение: это уже рассуждение о блюде, а не блюдо.
    """
    if not перевод:
        return False
    return len(перевод) <= max(90, len(оригинал) * 4)
